...
describe 'aws-translators', () ->
  ...
  describe '#deleteItem', () ->
    ...
    it 'should call send of aws with extra params', () ->
      sandbox.spy(dynastyTable.parent.dynamo, "send")
      options =
        ReturnValues: 'NONE'
      ...
      expect(dynastyTable.parent.dynamo.send.calledOnce)
      args = dynastyTable.parent.dynamo.send.getCall(0).args[0].input
      expect(args).to.include.keys('ReturnValues')
    ...
    it 'should send the table name to AWS', () ->
      ...
      promise.then () ->
        expect(dynastyTable.parent.dynamo.send.calledOnce)
        params = dynastyTable.parent.dynamo.send.getCall(0).args[0].input
