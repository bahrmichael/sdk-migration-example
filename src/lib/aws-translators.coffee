...
{ DeleteItemCommand, BatchGetItemCommand, GetItemCommand, ScanCommand, QueryCommand, PutItemCommand, UpdateItemCommand } = require('@aws-sdk/client-dynamodb')
...
module.exports.processAllPages = (deferred, dynamo, commandClass, params)->
  ...
  processPage = (currentParams) ->
    Promise.resolve(dynamo.send(new commandClass(currentParams)))
      .then (result) ->
        deferred.notify dataTrans.fromDynamo result.Items
        stats.Count += result.Count
        if result.LastEvaluatedKey
          currentParams.ExclusiveStartKey = result.LastEvaluatedKey
          processPage(currentParams)
        else
