...
{ DeleteItemCommand, BatchGetItemCommand, GetItemCommand, QueryCommand,
  ScanCommand, PutItemCommand, UpdateItemCommand } = require('@aws-sdk/client-dynamodb')
...
module.exports.processAllPages = (deferred, dynamo, commandClass, params)->
  ...
  resultHandler = (result) ->
    ...
    if result.LastEvaluatedKey
      ...
      dynamo.send(new commandClass(params)).then(resultHandler).catch((err) -> deferred.reject(err))
    else
