...
{ UpdateTableCommand, CreateTableCommand, DescribeTableCommand, DeleteTableCommand, ListTablesCommand } = require('@aws-sdk/client-dynamodb')
_ = require('lodash')
...
class Dynasty
  ...
  constructor: (credentials, url) ->
    ...
    if credentials.accessKeyId or credentials.secretAccessKey or credentials.sessionToken
      config.credentials = {}
      config.credentials.accessKeyId = credentials.accessKeyId || process.env.AWS_ACCESS_KEY_ID
      config.credentials.secretAccessKey = credentials.secretAccessKey || process.env.AWS_SECRET_ACCESS_KEY
      if credentials.sessionToken or process.env.AWS_SESSION_TOKEN
        config.credentials.sessionToken = credentials.sessionToken || process.env.AWS_SESSION_TOKEN
